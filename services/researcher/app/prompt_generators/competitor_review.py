...
def generate_prompt(params: PromptParams) -> PromptGeneratorResult:
    ...
    company_type = params["company_location"]
...
6. Особое внимание на {company_type} компании
"""
...
"""
    ...
    # Запрос для анализа
    prompt = (
        f"Подготовить {report_length} обзор конкурентов для {company}"
        + f" в {field} с учетом следующих критериев: {evaluation_criteria}\n"
        + f"Структура отчета: {structure}\nДля сотрудника: {employee_grade}. Ответ на русском языке."
    )
